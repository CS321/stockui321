...
/**
 * ClosedPositionView StructuredTable Format.
 *
 * Column 0) String Ticker. 
 * Column 1) Date buyDate. 
 * Column 2) Date closedDate
 * Column 3) int numShares. 
 * Column 4) double costBasis. 
 * Column 5) double realizedGains.
 *
 * @author treybriggs
 */
public class ClosedPositionView implements Strategy, Serializable {
    ...
    public <T extends Comparable<? super T>> StructuredTable genTable(StructuredTable transactions) {
        ...
        StructuredTable<T> c = new StructuredTable();
        ...
        while (stepThrough.hasNext()) {
            ...
            if ((TraderAccount.transactionType) srcRow.get(0) == TraderAccount.transactionType.BUY) {
                ...
                if (t.getEntry(index, 2).equals(numShares)) {
                    ...
                    // Closing Position, move data into new table and delete from this one
                    Date buyDate = (Date) t.getEntry(index, 1);
                    Date sellDate = (Date) srcRow.get(2);
                    ...
                    List destRow = new ArrayList();
                    destRow.add(tickerSymbol);
                    destRow.add(buyDate);
                    destRow.add(sellDate);
                    destRow.add(sharesSold);
                    destRow.add(costBasis);
                    destRow.add(realizedGains);
